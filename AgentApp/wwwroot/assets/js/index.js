$(function () {
    ...
    $.ajax({
        type: "POST",
        url: "/Agent/LoadAgentLineGraph",
        success: function (res) {
            debugger;
            if (res.statusCode === 200) {
                var rev = [];
                var _labels = [];
                for (var i = 0; i < res.object1.length; i++) {
                    _labels.push(res.object1[i].month);
                    rev.push(res.object1[i].count);
                }
                var options;
                var data = {
                    labels: _labels,//['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'],
                    series: [
                        rev
